...
2. **Case Generation (Easy / Medium / Hard)** ✅ — Selecting a difficulty spends coins (50 / 100 / 200), Groq generates a unique case file (victim, crime scene, 3–6 suspects, 2–3 witnesses) from a pool of **48 diverse settings** with culturally varied character names. Each suspect/witness carries a `gender` field for voice selection. Timer is set by `POST /case/:sessionId/begin` when the player clicks "Begin Investigation" in the case file (25 / 35 / 55 min) — reading time is not deducted from the play budget. Endpoint is idempotent: re-clicks return the existing `expiresAt` without extending. Tier rewards: Easy 150, Medium 300, **Hard 600** (flat 3× return).
...
4. **Hint System** ✅ — One hint per case; returns a subtle first-person nudge ("Something about Maria's alibi doesn't sit right…") drawn from a **10-opener pool** to vary phrasing across sessions; `HintAlreadyUsedError` thrown on a second request even under race conditions.
...
## Nice-to-have status
- [x] **Text-to-speech playback** — IMPLEMENTED. Per-page case file narration + full case briefing + per-character voice in interrogation. Gender-aware voice selection (`getBestFemaleVoice()` / `getBestMaleVoice()`). Deterministic per-character pitch (djb2 hash of name). Narration paced for comprehension (`NARRATOR_VOICE.rate = 0.88`, `STORY_VOICE.rate = 0.85`). All via Web Speech API — zero cost, zero API keys.
- [x] **Opt-in auto-narration** — IMPLEMENTED. `CaseFileBook` auto-fires `handleReadAll()` ~900 ms after mount **only when the user has opted in** via the `Auto` toggle. Default is OFF (`mm_case_file_autoplay` localStorage flag must be `'1'` to enable) — returning players get a quiet case file they can read at their own pace. TTS is also stopped on `handleBeginInvestigation` so any in-flight narration is killed when the player commits to gameplay.
...
2. **Generation repetition (same case / same hint twice)** — Mitigated by a 48-entry settings pool, 12-entry death seed pool, random entropy token (`Math.random + Date.now`) injected into every prompt, `temperature: 1.15 / top_p: 0.95` for case generation, and a 10-opener pool for hints with `temperature: 1.0 / top_p: 0.95`.
3. **Easy difficulty too hard** — Polish wave 3 rewrote the Easy rules with strict constraints: murderer's `why_suspect` is glaring, alibi has obvious internal contradiction, witnesses volunteer the contradiction, ≥3 initial evidence items with ≥2 pointing at the murderer, 0 red herrings. Heuristic: "a child detective could solve this with 2 questions."
4. **Cost blow-up** — Groq `llama-3.3-70b-versatile` is free-tier-friendly; no streaming. Each turn is a self-contained REST call. PostHog and Sentry are on free tiers with massive headroom.
5. **TTS voice quality** — Depends on the user's OS/browser voice list. Mitigated by a graceful fallback chain: gender-specific voice → neutral best voice → first available English voice. Narration rate explicitly tuned to 0.88 for comprehension.
6. **Timer abuse (re-clicking Begin Investigation to extend)** — Mitigated by an idempotency check on `POST /case/:id/begin` comparing `startedAt` vs `createdAt` with a 5-second tolerance.
7. **Image generation latency** — Crime scene image is async and non-blocking; the UI shows a placeholder while it loads.
8. **Production blind spots** — PostHog tracks the full funnel, Sentry catches 5xx in real time with authenticated user + request body context.
